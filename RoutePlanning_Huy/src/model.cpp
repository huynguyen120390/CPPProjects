...
/*
    Function to covert road in string to road in enum type
    @param : type [std::string_view] road type in string
    @return : Model::Road:Enum Type
*/
...
/*
    Function to covert land use in string to land use in enum type
    @param : type [std::string_view] land use type in string
    @return : Model::Landuse:Enum Type
*/
...
/*
    Construct receives arguments as byte vector of world map, 
    Note: [](const auto &_1st, const auto &_2nd){return (int)_1st.type <(int)_2nd.type;} is a comparison function, we can separate 
*/
...
void Model::LoadData(const std::vector<std::byte> &xml){
    //Use pugixml library to read xml
    using namespace pugi;
    xml_document doc;

    //If loading data from xml fails then throw std::logic_error
    if( !doc.load_buffer(xml.data(),xml.size()))
        throw std::logic_error("failed to parse the xml file");

    //Load boundaries of the map, if there is, take longtitude, latitude
    //atof(str) to convert from string to float
    //.as_string() to covert from byte to string
    //Note if statement in C++ can do such syntax as if(assignment,conditions){}
    ...
    //Create an unordered_map to store node pairs of (node id , node size)
    //Also push those nodes with latitude and longtitude to m_Nodes vector
    std::unordered_map<std::string,int> node_id_to_num;
    for(const auto &node:doc.select_nodes("/osm/node")){
        std::string nodeId = node.node().attribute("id").as_string();
        int nodeSize = (int)m_Nodes.size();
        node_id_to_num[nodeId] = nodeSize;
    ...
    for (const auto &way:doc.select_nodes("osm/way")){
        ...
        //A node in osm xml , in this case, includes 'nd' and 'tag', use node.children() to return all of them
        for(auto child: node.children()){
            auto name = std::string_view{child.name()};
            if(name == "nd"){
                auto ref = child.attribute("ref").as_string();
                //Go find a node in the node map , if found, add to nodes list of the way 
                // Note:aMap.find(something) , if not found, return the end() - the imagined element followed the last element
                if(auto it = node_id_to_num.find(ref); it != end(node_id_to_num)) 
                    new_way.nodes.emplace_back(it->second); //it-itertor is a pair (key,value) or (first,second)
            }else if (name == "tag"){
